...
import org.marketlogic.surveyappquery.backend.UserUpdateServiceImpl;
...
public class UserQueryController extends BaseController {
    ...
    public HttpEntity<Collection<ResourceWithUrl>> listAll() {
        List<ResourceWithUrl> resourceWithUrls = userUpdateService.getAll().stream().map(this::toResource).collect(Collectors.toList());
        return new ResponseEntity<>(resourceWithUrls, OK);
    }

    public ResourceWithUrl toResource(User user) {
